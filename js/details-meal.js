...
function displayRecipes(meal){
    ...
    for(let i = 0 ; i < 20 ; i++)
    {
        if(meal.meals['0'][`strIngredient${i+1}`] !== '')
        {
                box +=
            `
                <span> ${meal.meals['0'][`strMeasure${i+1}`]} ${meal.meals['0'][`strIngredient${i+1}`]}</span>
            `
        }
        else
            break;
...
function displayTags(meal){
    ...
    if(tags == null)
        ...
        tags = meal.meals['0'][`strTags`].split(',')
    ...
    `    
        <div class="btns mt-4">
            <a target="_blank" href="${meal.meals['0'][`strSource`]}" class="btn btn-success">Source</a>
            <a target="_blank" href="${meal.meals['0'][`strYoutube`]}" class="btn btn-danger">Youtube</a>
        </div>
    `;
